...
def _blocking_extract_info(url):
    opts = {
        ...
        'extractor_args': {
            ...
            'twitter': {'api': ['graphql', 'syndication']},
...
def _blocking_try_download_image(url):
    ...
    يرمي Exception لو الرابط مو صورة، عشان نرجع للسلوك القديم (أزرار فيديو/صوت).
    """
    opener = _get_urllib_opener()
    ...
    with opener.open(req, timeout=20) as resp:
        ...
        if not content_type.startswith('image/'):
            raise Exception(f"ليس رابط صورة مباشر (Content-Type: {content_type})")
        ext = content_type.split('/')[-1].split(';')[0].strip()
        if ext not in ('jpeg', 'jpg', 'png', 'webp', 'gif', 'bmp'):
            ext = 'jpg'
        out_path = f"zendown_img_{uuid.uuid4().hex[:8]}.{ext}"
        data = resp.read()
        if len(data) < 500:  # صفحة خطأ صغيرة مش صورة حقيقية غالباً
...
async def process_link_info(update: Update, context: ContextTypes.DEFAULT_TYPE, url: str):
    ...
    # لو فشل استخراج معلومات فيديو، جرب كون الرابط صورة مباشرة قبل ما نعرض أزرار فيديو/صوت اللي بتفشل أكيد
    if video_info_failed:
        try:
            ...
            pass  # مو صورة، كمل بالسلوك العادي (فيديو/صوت)
